...
public class RandomSpawnerCastle : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //find any objects with tag "Castle"
        GameObject[] castle = GameObject.FindGameObjectsWithTag("Road");
        //check if there are more castles than the length
        if (castle.Length > length)
        {
            Destroy(gameObject);
